    ...
    def simulate_one_avatar(self, avatar_id):
        ...
        # for i in range(2):
            ...
            recommended_items = ["<- " + item.title + " ->"
                            ...
                            for item in movies_on_page]
            recommended_items_str = "".join(recommended_items)
            avatar_.write_log(f"\n============    Recommendation Page {i+1}    ============")
            for idx, movie in enumerate(movies_on_page):
                tag = "(√) " if id_on_page[idx] in self.data.valid_user_list[avatar_id] else ""
                avatar_.write_log(f"== {tag}{movie.title} | rating={round(movie.rating,2)} | {movie.summary}")
            avatar_.write_log(f"============         End Page {i+1}         ============\n")

            #@ most important
            response = avatar_.reaction_to_recommended_items(recommended_items_str, i + 1)
            i += 1
