...
# activity_dict
    ...
    def init_memory(self):
        ...
        # Singleton: load embeddings model once across all avatars (was: re-load per avatar, ~5s each)
        if not hasattr(Avatar, "_shared_embeddings"):
            Avatar._shared_embeddings = HuggingFaceEmbeddings(model_name="sentence-transformers/all-MiniLM-L6-v2")
        embeddings_model = Avatar._shared_embeddings
        embedding_size = 384
        index = faiss.IndexFlatL2(embedding_size)
        vectorstore = FAISS(embeddings_model.embed_query, index, InMemoryDocstore({}), {}, relevance_score_fn=score_normalizer)

        LLM = ChatOpenAI(model_name="glm-4-plus", max_tokens=4000, temperature=0.3, request_timeout=90)
    ...
    def _reaction(self, messages=None, timeout=90):
        """
    ...
    def reaction_to_forced_items(self, recommended_items_str):
        ...
        reaction = self._reaction(messages, timeout=120)
    ...
    def reaction_to_recommended_items(self, recommended_items_str, current_page):
        ...
        self.write_log("\n" + sys_prompt, color="blue")
        self.write_log("\n" + prompt, color="blue")
        reaction = self._reaction(messages, timeout=120) # reaction
